...
def _build_supplemental_name_map(glicko_names: list) -> dict:
    ...
    all_suppl_names = sorted(
        set(df["winner_name"].dropna().unique())
        | set(df["loser_name"].dropna().unique())
    )
...
class PredictEngine:
    ...
    def __init__(self):
        ...
        self.player_form: dict = (
            {}
        )  # player_name → {form_3, form_5, form_15, form_50, surface_form_hard/clay/grass, win_rate_vs_top50}
    ...
    def load(self):
        ...
        if stacked_json.exists():
            ...
            logger.info(
                f"  ✓ Stacked ensemble loaded from JSON (coef shape: {stacker.coef_.shape})"
            )
        else:
        ...
        for _pname, surfaces in self.glicko.ratings.items():
            ...
            if (
                r
                and r.last_match_date
                and (max_date is None or r.last_match_date > max_date)
            ):
        ...
        if max_date:
            ...
            logger.info(
                f"  ✓ Latest data date: {LATEST_DATA_DATE} (from max last_match_date)"
            )
        else:
        ...
        if SUPPLEMENTAL_CSV.exists():
            try:
                ...
                if sup_date > LATEST_DATA_DATE:
                    ...
                    logger.info(
                        f"  ✓ Latest data date updated to {LATEST_DATA_DATE} (from supplemental CSV)"
                    )
            except Exception as e:
    ...
    def _compute_attribute_proxies(self):
        ...
        all_names = sorted(
            set(pts["Player 1"].unique()) | set(pts["Player 2"].unique())
        )
        processed = 0
        ...
        for player_name in all_names:
            ...
            if "rally_length" in pp.columns:
                ...
                if len(long_rallies) >= 50:
                    long_won = (
                        (long_rallies["PtWinner"] == 1)
                        & (long_rallies["Player 1"] == player_name)
                    ) | (
                        (long_rallies["PtWinner"] == 2)
                        & (long_rallies["Player 2"] == player_name)
                    )
            ...
            if "last_shot_type" in pp.columns:
                ...
                if len(net_pts) >= 30:
                    net_won = (
                        (net_pts["PtWinner"] == 1)
                        & (net_pts["Player 1"] == player_name)
                    ) | (
                        (net_pts["PtWinner"] == 2)
                        & (net_pts["Player 2"] == player_name)
                    )
    ...
    def _get_feature_cols(self) -> list:
        ...
        return [
            ...
            "p1_acute_stress_7d",
            "p1_acute_minutes_7d",
            "p1_acute_match_count_7d",
            "p1_chronic_stress_28d",
            "p1_chronic_minutes_28d",
            "p1_chronic_match_count_28d",
            "p1_acwr",
            "p1_stress_14d",
            "p1_minutes_14d",
            "p1_days_rest",
            "p1_last_match_minutes",
            "p1_last_match_stress",
            "p1_last_opponent_grinding",
            "p1_last_match_competitiveness",
            "p1_consecutive_match_days",
            "p1_consecutive_day_stress",
            "p1_tournament_load_30d",
            "p1_avg_recent_grinding",
            "p1_avg_recent_competitiveness",
            "p1_surface_consistency_recent",
            "p2_acute_stress_7d",
            "p2_acute_minutes_7d",
            "p2_acute_match_count_7d",
            "p2_chronic_stress_28d",
            "p2_chronic_minutes_28d",
            "p2_chronic_match_count_28d",
            "p2_acwr",
            "p2_stress_14d",
            "p2_minutes_14d",
            "p2_days_rest",
            "p2_last_match_minutes",
            "p2_last_match_stress",
            "p2_last_opponent_grinding",
            "p2_last_match_competitiveness",
            "p2_consecutive_match_days",
            "p2_consecutive_day_stress",
            "p2_tournament_load_30d",
            "p2_avg_recent_grinding",
            "p2_avg_recent_competitiveness",
            "p2_surface_consistency_recent",
            "acute_stress_7d_diff",
            "chronic_stress_28d_diff",
            "acwr_diff",
            "days_rest_diff",
            "consecutive_match_days_diff",
            "minutes_14d_diff",
            "last_match_stress_diff",
            "tournament_load_30d_diff",
            "fatigue_asymmetry",
            "p1_charted_match_count",
            "p1_charted_serve_entropy",
            "p1_charted_first_strike_rate",
            "p1_charted_pressure_divergence",
            "p1_charted_aggression_index",
            "p1_charted_long_rally_wr",
            "p1_charted_rally_win_slope",
            "p1_charted_pattern_diversity",
            "p1_charted_avg_rally_length",
            "p2_charted_match_count",
            "p2_charted_serve_entropy",
            "p2_charted_first_strike_rate",
            "p2_charted_pressure_divergence",
            "p2_charted_aggression_index",
            "p2_charted_long_rally_wr",
            "p2_charted_rally_win_slope",
            "p2_charted_pattern_diversity",
            "p2_charted_avg_rally_length",
            "charted_serve_entropy_diff",
            "charted_first_strike_rate_diff",
            "charted_pressure_divergence_diff",
            "charted_aggression_index_diff",
            "charted_long_rally_wr_diff",
            "charted_rally_win_slope_diff",
            "charted_pattern_diversity_diff",
            "charted_avg_rally_length_diff",
            "style_clash",
            "p1_form_3",
            "p1_form_5",
            "p1_form_15",
            ...
            "p1_df_rate",
            "p1_first_serve_pct",
            "p1_first_serve_won_pct",
            "p1_second_serve_won_pct",
            "p1_bp_save_pct",
            "p1_win_rate_vs_top50",
            "p2_form_3",
            "p2_form_5",
            "p2_form_15",
            "p2_form_50",
            ...
            "p2_df_rate",
            "p2_first_serve_pct",
            "p2_first_serve_won_pct",
            "p2_second_serve_won_pct",
            "p2_bp_save_pct",
            "p2_win_rate_vs_top50",
            "form_3_diff",
            "form_5_diff",
            "form_15_diff",
            "form_50_diff",
            ...
            "bp_save_pct_diff",
            "win_rate_vs_top50_diff",
            "heat_x_fatigue",
            "heat_x_fatigue_diff",
            "wind_x_serve_dep",
            "humidity_x_rally",
            "heat_x_endurance_diff",
            # H2H features
            "h2h_wins_p1",
            "h2h_wins_p2",
            "h2h_total",
    ...
    def _build_form_cache(self):
        ...
        matches = (
            matches.dropna(subset=["date"]).sort_values("date").reset_index(drop=True)
        )
    ...
    def _build_match_caches(self):
        ...
        def _process_match(w, l, surf, tdate, w_age=None, l_age=None):
            ...
            if surf:
                ...
                entry["surface_wins"][surf][w] = (
                    entry["surface_wins"][surf].get(w, 0) + 1
                )
        ...
        for csv_path in csv_files:
            ...
            for _, row in df.iterrows():
                ...
                tdate = (
                    int(row["tourney_date"]) if pd.notna(row.get("tourney_date")) else 0
                )
                w_age = (
                    float(row["winner_age"])
                    if pd.notna(row.get("winner_age"))
                    else None
                )
                l_age = (
                    float(row["loser_age"]) if pd.notna(row.get("loser_age")) else None
                )
    ...
    def find_player(self, query: str) -> Optional[str]:
        ...
        close_last = difflib.get_close_matches(
            q, list(last_names.keys()), n=1, cutoff=0.6
        )
        if close_last:
    ...
    def _get_serve_stats(self, player_name: str) -> dict:
        ...
        return {
            ...
            "first_serve_pct": safe_div(
                acc.total_1st_serves_in, acc.total_1st_serve_attempts, 0.620
            ),
            "first_serve_won_pct": safe_div(
                acc.total_1st_serve_won, acc.total_1st_serve_played, 0.720
            ),
            "second_serve_won_pct": safe_div(
                acc.total_2nd_serve_won, acc.total_2nd_serve_played, 0.520
            ),
    ...
    def _get_charted_stats(self, player_name: str) -> dict:
        ...
        return {
            ...
            "charted_serve_entropy": acc.serve_direction_entropy_sum
            / max(1, acc.serve_direction_matches),
            "charted_first_strike_rate": acc.total_first_strike_won
            / max(1, acc.total_first_strike_points),
            "charted_pressure_divergence": pressure_div,
            "charted_aggression_index": acc.aggression_sum / max(1, n_chart),
            "charted_long_rally_wr": acc.long_rally_wins
            / max(1, acc.long_rally_points),
    ...
    def build_feature_vector(
        ...
        c_fstrike_diff = (
            p1c["charted_first_strike_rate"] - p2c["charted_first_strike_rate"]
        )
        c_press_diff = (
            p1c["charted_pressure_divergence"] - p2c["charted_pressure_divergence"]
        )
        ...
        row = {
            ...
            "first_serve_won_pct_diff": p1s["first_serve_won_pct"]
            - p2s["first_serve_won_pct"],
            "second_serve_won_pct_diff": p1s["second_serve_won_pct"]
            - p2s["second_serve_won_pct"],
    ...
    def predict(
        ...
        if isinstance(ens, dict) and ens.get("type") == "stacked":
            ...
            stacker = ens["stacker"]
            base_probs = [xgb_prob if n == "xgboost" else lgb_prob for n in model_names]
            ...
            preds = {"xgboost": np.array([xgb_prob]), "lightgbm": np.array([lgb_prob])}
    ...
    def get_player_card(self, player_name: str, surface: str = "hard") -> dict:
        ...
        def _compute_card(mu, peak_mu, rd, is_ret, form_3_val):
            ...
            base_tier = (
                "legendary"
                if base >= 91
                else "gold" if base >= 80 else "silver" if base >= 69 else "bronze"
            )
            return {
                ...
                "glow_direction": (
                    "up" if display > base else "down" if display < base else "none"
                ),
            }
        ...
        return {
            ...
            "last_match": (
                str(p1g["last_match_date"]) if p1g["last_match_date"] else None
            ),
...
def _build_country_cache():
    ...
    for year in range(2020, 2025):
        ...
        try:
            df = pd.read_csv(
                csv_path,
                usecols=["winner_name", "winner_ioc", "loser_name", "loser_ioc"],
            )
            for _, row in df.iterrows():
                if row["winner_name"] and not pd.isna(row["winner_name"]):
                    _COUNTRY_CACHE[row["winner_name"]] = str(
                        row.get("winner_ioc", "UNK") or "UNK"
                    )
                if row["loser_name"] and not pd.isna(row["loser_name"]):
                    _COUNTRY_CACHE[row["loser_name"]] = str(
                        row.get("loser_ioc", "UNK") or "UNK"
                    )
        except Exception:
            pass
