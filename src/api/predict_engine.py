...
class PredictEngine:
    ...
    def predict(
        ...
        return {
            ...
            'surface':          surface,
            'player1_win_prob': round(p1_win_prob, 3),
            'player2_win_prob': round(p2_win_prob, 3),
            'confidence':       confidence,
            'confidence_reason': confidence_reason,
            'model':            'stacked (XGB+LGB)',
            'xgb_prob':         round(xgb_prob, 3),
            'lgb_prob':         round(lgb_prob, 3),
            'elo_diff':         round(p1g['mu_all'] - p2g['mu_all'], 1),
            'elo_surface_diff': round(p1g['mu_surf'] - p2g['mu_surf'], 1),
            'rd_p1':            round(p1g['rd_all'], 1),
            'rd_p2':            round(p2g['rd_all'], 1),
            'p1_glicko':        round(p1g['mu_all'], 0),
            'p2_glicko':        round(p2g['mu_all'], 0),
        }
